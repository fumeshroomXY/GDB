# How to start GDB
```bash
gcc -g main.c -o prog   # -g means including debug information
gdb ./prog              # start debugging the target program
```
Or we can load the file in GDB:
```bash-g means “include debug information
gdb
(gdb) file ./prog
```
